...
- 以 `#` 开头的行为注释，忽略。
- 空行忽略。
- 本节是简易输入格式，不构成软件能力白名单。软件支持当前 sing-box 核心的完整原生规则语法，输入方式见第 15 节；简易类型映射及兼容差异统一维护在 Wiki。
...
## 15. 完整原生语法与 Wiki 分工（修订）

- 自定义规则集是 TungBox 的应用层分流方案，区别于 sing-box 原生 rule-set。后者提供匹配条件，由路由规则指定动作；不新增方案之间的隐式递归引用。
- 保留简易模式：多行 `TYPE, VALUE` 加统一出站，各行独立匹配，通常为 OR 关系。它不表达“某域名且端口 443”这样的复合条件。
- 增加原生模式：直接输入完整 `route.rules` JSON 对象数组，支持当前核心的全部匹配字段、逻辑组合、反选、动作及嵌套语法。保留顺序和所有字段，不用现有类型下拉或硬编码字段集合限制能力。
- 原生模式由每条规则自己指定 action/outbound，隐藏统一出站控件，不覆盖用户动作；省略字段时遵循核心默认语义。简易到原生转换必须无损，无法无损回转时保持原生模式。
- 原生规则依赖的出站、DNS server、原生 rule-set 等由完整配置提供。完整配置和 DNS 规则仍在相应编辑入口维护；支持全部语法不意味着把不同层级的配置放进 route.rules。
- 软件默认开放当前所选核心支持的语法，由实际核心 `sing-box check` 校验完整候选配置；未知新字段不能仅因 UI 未收录而被删除或拒绝。
- 映射表、兼容写法和示例放在 Wiki：覆盖 URL 与域名正则差异、通配符、GEOIP 资源引用、LAN、协议嗅探、Clash 三段格式及 no-resolve。不能无损转换的简易写法提示改用原生模式，禁止静默近似转换或吞掉额外参数。
- Wiki 同时说明域名规则依赖域名信息、协议规则依赖嗅探，以及路由出站选择不等同于 DNS 服务器选择。软件内保留说明链接，不增加规则类型白名单。
...
- 只按应用掌握的来源和用途安排准备规则；订阅或用户原生规则内部顺序保持不变，不任意抽取所有 sniff/resolve 提前执行。未知动作交由核心解释，不当作准备动作跳过。
...
- 第 3 节模型视为旧格式。新格式增加 schemaVersion、format（simple/native）和 order；简易模式保存 entries 与统一策略，原生模式保存完整 JSON 对象数组，不用穷举字段模型损失数据。
- 持续使用当前实现的 `<ruleSetID-UUID>.yml` 文件名，重命名只改变名称，文件名与 UUID 不变，替代初版 slug 方案。
- 新格式 YAML 保存元数据；simple 模式使用 outbound/rules，native 模式用 `nativeRulesJSON: |` 块保存 JSON 数组，二者互斥。原生载荷不经过简易行解析器。
- 缺少 schemaVersion 的旧文件按明确迁移规则处理。缺失或非法 UUID 不得每次随机生成；如需补全必须一次性保存稳定结果。缺少 rules 与显式空清单不同。
- 检查必填字段、重复键、字段类型、同订阅重名和重复 UUID；enabled 只接受布尔值。明确 YAML 引号、注释、转义、换行和块标量行为，禁止默默忽略损坏结构。
- 不再强制手写解析器和零依赖：解析方案必须支持既定格式并通过往返测试。未知元数据保留或明确报错，原生规则字段必须无损保留。
- 简易 CIDR 验证真实地址、地址族和前缀长度，不能只检查斜杠；正则以实际核心接受为准。错误提供方案名和行号或 JSON 路径。
- 当前更新器接受 1.12.x / 1.13.x，这是实现现状，不是永久语法上限。能力以实际核心版本和构建为准；升级或切换后重新检查完整配置。核心缺失时只能保留待验证草稿，不宣称校验通过。

## 19. 验收补充

- 原生逻辑组合、反选、非终结动作、UI 未列字段及嵌套结构无损往返；模式转换不丢数据，由支持相应语法的核心校验。
- 简易格式非法 CIDR、正则、额外参数正确报错；原生错误路径可定位。空方案不生成兜底匹配规则。
- YAML 旧格式迁移、缺字段、重复键／UUID／名称、非法布尔值、特殊字符与块标量覆盖。
...
- Wiki 映射表、原生示例、兼容差异、嗅探／DNS 前提及失效回落说明完成，软件内链接可达。

## 20. 实施顺序与参考

1. 基础配置来源、迁移与重建；动作和优先级回归。
2. 原生输入、无损模型、文件迁移、引用校验与失效状态。
